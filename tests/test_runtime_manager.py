...
def test_manifest_no_deps_package_changes_runtime_fingerprint(tmp_path, monkeypatch):
    module = __import__("studio.runtime_manager", fromlist=["RUNTIME_MANIFESTS"])
    manifest = RuntimeManifest(
        runtime_id="no-deps-fingerprint",
        display_name="No Deps Fingerprint",
        install_mode=RuntimeInstallMode.ISOLATED,
        requirements=("shared-dependency==1.0",),
        no_deps_requirements=("engine-package==1.0",),
    )
    monkeypatch.setitem(module.RUNTIME_MANIFESTS, manifest.runtime_id, manifest)
    manager = RuntimeManager(tmp_path / "runtimes")
    python_path = _venv_python(manager.environment_path(manifest.runtime_id))
    python_path.parent.mkdir(parents=True, exist_ok=True)
    python_path.write_bytes(b"fake")
    manager._record_install(manifest.runtime_id, "test")
    assert manager.status(manifest.runtime_id).ready is True
    ...
    changed = RuntimeManifest(
        ...
        install_mode=RuntimeInstallMode.ISOLATED,
        requirements=manifest.requirements,
        no_deps_requirements=("engine-package==2.0",),
    )
    monkeypatch.setitem(module.RUNTIME_MANIFESTS, manifest.runtime_id, changed)
    assert manager.status(manifest.runtime_id).ready is False
...
def test_no_deps_engine_package_installs_last_with_dependency_resolution_disabled(tmp_path, monkeypatch):
    module = __import__("studio.runtime_manager", fromlist=["RUNTIME_MANIFESTS"])
    manifest = RuntimeManifest(
        runtime_id="no-deps-order",
        display_name="No Deps Order",
        install_mode=RuntimeInstallMode.ISOLATED,
        requirements=("shared-dependency==1.0",),
        no_deps_requirements=("engine-package==1.0",),
        bootstrap_requirements=("torch==2.13.0",),
    ...
    monkeypatch.setattr(module.subprocess, "run", fake_run)
    assert manager.install(manifest.runtime_id).ready is True

    assert len(calls) == 4
    assert calls[1][-1] == "torch==2.13.0"
    assert calls[2][-1] == "shared-dependency==1.0"
    assert calls[3][-1] == "engine-package==1.0"
    assert "--no-deps" in calls[3]
    assert "--no-deps" not in calls[2]
